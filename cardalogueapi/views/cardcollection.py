...
class CardCollectionView(ViewSet):
    def list (self, request):
        """handes GET all"""
        cc = CardCollection.objects.all()
        serializer = CardCollectionSerializer(cc, many=True)
        return Response(serializer.data)

    def retrieve(self, request, pk):
        """handles GET single"""
        cc = CardCollection.objects.get(pk=pk)
        serializer = CardCollectionSerializer(cc)
        return Response(serializer.data)
    ...
    def update(self, request, pk):
        ...
        """
        cc = CardCollection.objects.get(pk=pk)
        serializer = CreateCardCollectionSerializer(cc, data=request.data)
        serializer.is_valid(raise_exception=True)
        serializer.save()
